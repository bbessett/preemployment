(function(Vue){
  ...
  document.addEventListener("DOMContentLoaded", function(){
    ...
    if( document.getElementsByClassName('grid-filter__container').length > 0 ) {
      gridId = document.getElementsByClassName('grid-filter__container')[0].id;
    }
    ...
    if( document.getElementById('past-event-grid') && window.eventData && window.eventFilters) {
 ...
 /*
  ...
  Vue.component('filter-nav-item', {
     ...
     * @prop string active Currently selected filter to set active state
     */
  ...
  Vue.component('filter-mobile-nav', {
    ...
    template: '<div class="filter-nav-mobile select-filter"> \
                 ...
                 <div class="select-filter__label">{{ titleForValue }} \
               </div>'
  });
  ...
  Vue.component('grid-list-item', {
    props: ['item', 'debug'],
    template: '<li class="grid-list__item col-md-6 col-lg-4"> \
  ...
  Vue.component('grid-list-item-video', {
    props: ['item', 'debug'],
    template: '<div class="block-grid col-md-6 col-lg-4"> \
      <a class="link--block modalLink" v-bind:area-label="item.title" v-bind:data-target="item.id" > \
        <div class="block-grid__media"> \
          <img :src="item.imagePath" :alt="item.title"/> \
        </div> <!-- block-grid__media --> \
      </a> \
      <p v-html="item.title"></p> \
      <div class="modal-overlay" :id="item.id" aria-hidden="true" tabindex="-1" role="dialog" v-bind:area-label="item.title"> \
      <a class="cancel"></a> \
      <div class="modal" role="document"> \
          <div class="modal-content"> \
            <div class="close"> \
              <a aria-label="Close" aria-hidden="true"> \
                <img alt="Close" src="/resources/img/icons/xIcon.svg"> \
              </a> \
            </div> \
              <div class="responsive-iframe-wrapper modal-body"> \
                  <iframe :src="item.imagePath" :title="item.title" :data-src="item.url" webkitallowfullscreen mozallowfullscreen \
